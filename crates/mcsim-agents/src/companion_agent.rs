//! Companion Agent for MeshCore companion firmware.
//!
//! This agent communicates with MeshCore companion firmware using the binary
//! UART protocol. Unlike the CLI agent which uses a text-based protocol for
//! repeaters and room servers, the companion agent handles direct messages
//! and channel messages through the companion firmware.
//!
//! The companion agent:
//! 1. Initializes the protocol with DeviceQuery/AppStart
//! 2. Sets up channels and contacts
//! 3. Sends direct messages and/or channel messages based on configuration
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CompanionAgentConfig {
...
impl Default for CompanionAgentConfig {
    fn default() -> Self {
        CompanionAgentConfig {
            name: "CompanionAgent".to_string(),
...
pub enum CompanionProtocolState {
...
// ============================================================================
// Companion Agent Entity
// ============================================================================

/// Companion agent entity that communicates with MeshCore companion firmware.
///
/// This agent uses the binary UART protocol to send direct messages and/or
/// channel messages through companion firmware.
pub struct CompanionAgent {
    ...
    protocol_state: CompanionProtocolState,
...
impl CompanionAgent {
    ...
    pub fn new(
        ...
        CompanionAgent {
            ...
            protocol_state: CompanionProtocolState::Uninitialized,
            channels_setup: 0,
            contacts_setup: 0,
            direct_state,
            direct_target_idx: 0,
    ...
    pub fn config(&self) -> &CompanionAgentConfig {
        &self.config
    }
    ...
    pub fn protocol_state(&self) -> CompanionProtocolState {
        self.protocol_state
    }
    ...
    fn send_command(&self, ctx: &mut SimContext, cmd: &Command) {
        ...
        trace!("CompanionAgent[{}]: Sending command {:?} ({} bytes)", self.config.name, cmd.code(), frame.len());
        ctx.post_immediate(
    ...
    fn start_initialization(&mut self, ctx: &mut SimContext) {
        debug!("CompanionAgent[{}]: Starting protocol initialization", self.config.name);
        self.send_command(ctx, &Command::DeviceQuery { app_version: 8 });
        self.protocol_state = CompanionProtocolState::AwaitingDeviceInfo;
    }

    /// Continue initialization after DeviceInfo.
    fn send_app_start(&mut self, ctx: &mut SimContext) {
        debug!("CompanionAgent[{}]: Sending AppStart", self.config.name);
        self.send_command(
            ctx,
            &Command::AppStart {
                reserved: [0u8; 7],
        ...
        self.protocol_state = CompanionProtocolState::AwaitingAppStart;
    }
    ...
    fn setup_channels(&mut self, ctx: &mut SimContext) {
        ...
        if total_channels == 0 {
            ...
            debug!("CompanionAgent[{}]: No channels to configure, setting up contacts", self.config.name);
        ...
        self.protocol_state = CompanionProtocolState::SettingUpChannels;
    ...
    fn send_next_channel_setup(&mut self, ctx: &mut SimContext) {
        ...
        if self.channels_setup >= total_channels {
            ...
            debug!("CompanionAgent[{}]: All {} channels configured, setting up contacts", 
        ...
        debug!("CompanionAgent[{}]: Setting up channel {} at index {}", 
    ...
    fn setup_contacts(&mut self, ctx: &mut SimContext) {
        if self.config.contacts.is_empty() {
            ...
            debug!("CompanionAgent[{}]: No contacts to configure, going to Ready", self.config.name);
            self.protocol_state = CompanionProtocolState::Ready;
            self.on_ready(ctx);
            return;
        }

        // Start setting up contacts
        debug!("CompanionAgent[{}]: Setting up {} contacts", self.config.name, self.config.contacts.len());
        self.contacts_setup = 0;
        self.protocol_state = CompanionProtocolState::SettingUpContacts;
    ...
    fn query_contacts(&mut self, ctx: &mut SimContext) {
        debug!("CompanionAgent[{}]: Querying contacts from firmware", self.config.name);
        self.protocol_state = CompanionProtocolState::QueryingContacts;
    ...
    fn send_next_direct_message(&mut self, ctx: &mut SimContext) {
        ...
        if let Some(limit) = self.config.direct.message_count {
            if self.direct_messages_sent >= limit {
                debug!("CompanionAgent[{}]: Direct message count limit reached ({})", self.config.name, limit);
        ...
        if self.config.direct.targets.is_empty() {
            warn!("CompanionAgent[{}]: No direct message targets configured", self.config.name);
    ...
    fn handle_direct_timeout(&mut self, ctx: &mut SimContext) {
        if let DirectMessageState::WaitingAck { .. } = self.direct_state {
            debug!("CompanionAgent[{}]: Direct message ACK timeout", self.config.name);
            self.schedule_next_direct_or_session(ctx);
        }
    }
    ...
    fn send_next_channel_message(&mut self, ctx: &mut SimContext) {
        ...
        if let Some(limit) = self.config.channel.message_count {
            if self.channel_messages_sent >= limit {
                debug!("CompanionAgent[{}]: Channel message count limit reached ({})", self.config.name, limit);
        ...
        if self.config.channel.targets.is_empty() {
            warn!("CompanionAgent[{}]: No channel targets configured", self.config.name);
        ...
        debug!(
            "CompanionAgent[{}]: Sending to channel {}: {}",
            self.config.name,
            channel_idx,
            content
        );
    ...
    fn handle_response(&mut self, resp: Response, ctx: &mut SimContext) {
        match resp {
            Response::DeviceInfo(_info) => {
                debug!("CompanionAgent[{}]: Received DeviceInfo, sending AppStart", self.config.name);
                self.protocol_state = CompanionProtocolState::DeviceInfoReceived;
                self.send_app_start(ctx);
            }
            Response::SelfInfo(info) => {
                debug!(
                    "CompanionAgent[{}]: Received SelfInfo, pubkey={:?}",
                    self.config.name,
            ...
            Response::Ok => {
                ...
                if self.protocol_state == CompanionProtocolState::SettingUpChannels {
                    self.channels_setup += 1;
                    self.send_next_channel_setup(ctx);
                } else if self.protocol_state == CompanionProtocolState::SettingUpContacts {
                    self.contacts_setup += 1;
                    self.send_next_contact_setup(ctx);
                } else {
                    trace!("CompanionAgent[{}]: OK response", self.config.name);
                }
            }
            Response::Sent { expected_ack, .. } => {
                debug!(
                    "CompanionAgent[{}]: Message sent, ack_hash=0x{:08x}",
            ...
            Response::Error(code) => {
                warn!("CompanionAgent[{}]: Error response: {:?}", self.config.name, code);
            ...
            Response::ContactsStart { total_count } => {
                if self.protocol_state == CompanionProtocolState::QueryingContacts {
                    info!("CompanionAgent[{}]: Firmware reports {} contacts in GetContacts response", self.config.name, total_count);
                }
            }
            Response::Contact(contact) => {
                if self.protocol_state == CompanionProtocolState::QueryingContacts {
                    trace!("CompanionAgent[{}]: Contact in firmware: '{}' type={} path_len={}",
                        self.config.name, contact.name, contact.contact_type, contact.out_path_len);
                }
            }
            Response::EndOfContacts { most_recent_lastmod } => {
                if self.protocol_state == CompanionProtocolState::QueryingContacts {
                    info!("CompanionAgent[{}]: EndOfContacts received (lastmod={}), going to Ready", 
                        self.config.name, most_recent_lastmod);
                    self.protocol_state = CompanionProtocolState::Ready;
                    self.on_ready(ctx);
                } else {
                    warn!("CompanionAgent[{}]: Received EndOfContacts in unexpected state {:?}",
                        self.config.name, self.protocol_state);
                }
            }
            _ => {
                trace!("CompanionAgent[{}]: Unhandled response: {:?}", self.config.name, resp);
            }
        }
    }
    ...
    fn handle_contact_message(&mut self, msg: ReceivedContactMessage, ctx: &mut SimContext) {
        ...
        debug!(
            "CompanionAgent[{}]: Received DM from {:?}: {}",
            self.config.name,
    ...
    fn handle_channel_message(&mut self, msg: ReceivedChannelMessage, _ctx: &mut SimContext) {
        ...
        debug!(
            "CompanionAgent[{}]: Received channel {} message: {}",
            self.config.name,
            msg.channel_idx,
            msg.text
        );
    ...
    fn handle_push(&mut self, push: PushNotification, ctx: &mut SimContext) {
        match push {
            PushNotification::SendConfirmed { ack_hash, trip_time_ms } => {
                debug!(
                    "CompanionAgent[{}]: ACK confirmed, hash=0x{:08x}, rtt={}ms",
            ...
            PushNotification::Advert { public_key } => {
                debug!(
                    "CompanionAgent[{}]: Advert received from {:?}",
                    self.config.name,
                    public_key.to_hex()
                );
            }
            _ => {
                trace!("CompanionAgent[{}]: Unhandled push: {:?}", self.config.name, push);
            }
        }
    }
}

impl Entity for CompanionAgent {
            ...
            EventPayload::Timer { timer_id } => {
                match *timer_id {
                    TIMER_PROTOCOL_INIT => {
                        ...
                        if self.protocol_state == CompanionProtocolState::Uninitialized {
                            self.start_initialization(ctx);
                        }
                    }
                    TIMER_DIRECT_STARTUP => {
                        // Direct messaging startup complete
                        if self.protocol_state == CompanionProtocolState::Ready {
                    ...
                    TIMER_DIRECT_SHUTDOWN => {
                        ...
                        if self.direct_state != DirectMessageState::Disabled 
                            && self.direct_state != DirectMessageState::Shutdown 
                        {
                            debug!("CompanionAgent[{}]: Direct message shutdown timer reached", self.config.name);
                            self.direct_state = DirectMessageState::Shutdown;
                        }
                    }
                    TIMER_CHANNEL_SHUTDOWN => {
                        ...
                        if self.channel_state != ChannelMessageState::Disabled 
                            && self.channel_state != ChannelMessageState::Shutdown 
                        {
                            debug!("CompanionAgent[{}]: Channel message shutdown timer reached", self.config.name);
                            self.channel_state = ChannelMessageState::Shutdown;
                        }
                    }
                    _ => {}
                ...
                loop {
                    match self.protocol_session.try_decode() {
                        ...
                        Err(e) => {
                            ...
                            error!(
                                "CompanionAgent[{}]: Protocol decode error: {}. Resetting session. \
...
/// Create a new companion agent.
pub fn create_companion_agent(
    id: EntityId,
    config: CompanionAgentConfig,
    attached_node: NodeId,
    attached_firmware: EntityId,
) -> CompanionAgent {
    CompanionAgent::new(id, config, attached_node, attached_firmware)
}
...
mod tests {
    ...
    #[test]
    fn test_companion_agent_config_default() {
        let config = CompanionAgentConfig::default();
